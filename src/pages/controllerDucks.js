...
export const signIn =()=> {
    return async (dispatch, getState) => {
        ...
        Auth.signIn(state.controller.loginUserID, state.controller.loginUserPassword)
            .then(user => {
...
export const signUp =()=>{
    return async (dispatch, getState)=>{
        let state = getState();
        if (state.controller.loginUserID !== '' && state.controller.loginUserPassword !== '')
        {
            console.log("signup is requested .... ");
            dispatch({type: SIGN_UP_USER_START})
            Auth.signUp(
                state.controller.loginUserID,
                state.controller.loginUserPassword,
                state.controller.loginUserID,
            )
                .then(user => {
...
export const confirmSignUp =()=>{
    return (dispatch, getState)=>{
        ...
        if (state.controller.loginUserID !== '' && state.controller.loginUserPassword !== '')
        {
            ...
            Auth.confirmSignUp(state.controller.loginUserID, state.controller.verificationCode,
                {
...
export const forgotPassword =()=>{
    return (dispatch, getState)=>{
        let state = getState();  // state
        if (state.controller.loginUserID !== '')
        {
            console.log("forget Password request .... ");
            dispatch({type : FORGOT_PASSWORD_START});
            Auth.forgotPassword(state.controller.loginUserID)
                .then(data =>  {
...
export const sendNewPassword =()=>{
    return (dispatch, getState)=>{
        let state = getState();  // state
        if (state.controller.loginUserID !== '' && state.controller.loginUserPassword !== '' && state.controller.verificationCode !== '')
        {
            console.log("signup-confirm is requested .... ");
            dispatch({type : SUBMIT_PASSWORD_START});
            Auth.forgotPasswordSubmit(state.controller.loginUserID, state.controller.verificationCode, state.controller.loginUserPassword)
                .then(data =>  {
