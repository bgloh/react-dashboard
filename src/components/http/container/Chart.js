...
import {setDateType, setViewType} from "../httpDucks";
...
const SetViewType = () => {//데이터 표시 방식에 따라 다른 데이터를 받아 차트 생성
    ...
    const data = useSelector((state) => state.http.chart)
    const datalist = []
    if(viewType === 'all')
        ...
        <div>
            <div style={{float : "right"}}>
                <Dropdown//DropDown으로 그래프에 표시 될 데이터 선택
                    ...
                    selection
                    options={viewTypes}
                    onChange={(e, {value }) => dispatch(setViewType(value))}
                />
                {"   "}
                <Dropdown//DropDown으로 그래프에 표시 될 데이터의 기준 선택
                    ...
                    selection
                    options={dateTypes}
                    onChange={(e, {value }) => dispatch(setDateType(value))}
                />
            </div>
            <br/>
            <br/>
